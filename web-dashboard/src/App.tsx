...
export default function App() {
  ...
  // Filter Tasks
  const filteredTasks =
    selectedProject?.tasksStore?.tasks.filter((t) => {
          ...
          {loading ? (
            <div style={{ display: 'grid', placeItems: 'center', height: '200px' }}>
              ...
              >
                ...
                <span className="pulse" style={{ color: '#94a3b8' }}>
                  Buscando archivos de proyectos en Google Drive...
                </span>
              </div>
            </div>
          ) : projects.length === 0 ? (
            ...
            >
              ...
              <p style={{ fontSize: '1.1rem', marginBottom: '8px' }}>
                No se encontraron archivos de Paratext Project Manager.
              </p>
              <p style={{ fontSize: '0.9rem', color: '#64748b' }}>
                Asegúrese de haber habilitado y sincronizado Google Drive desde la extensión de
                Paratext 10.
              </p>
              <button
                  ...
                  <div>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        {proj.tasksFileId ? '✓ Disponible' : '❌ No configurado'}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <GitPullRequest size={14} /> Pull Requests:{' '}
                        {proj.prsFileId ? '✓ Disponible' : '❌ No configurado'}
                      </div>
                    </div>
                  </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              ...
              <p style={{ color: '#94a3b8', fontSize: '0.85rem' }}>
                Código del proyecto: {selectedProject.id}
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <span style={{ color: '#94a3b8', fontSize: '0.9rem' }}>{username} (Consultor)</span>
            <button
          ...
          >
            <button
              ...
              style={{
                ...
                padding: '8px 16px',
                border: 'none',
                background: activeTab === 'board' ? undefined : 'transparent',
                boxShadow: activeTab === 'board' ? undefined : 'none',
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                padding: '8px 16px',
                border: 'none',
                background: activeTab === 'prs' ? undefined : 'transparent',
                boxShadow: activeTab === 'prs' ? undefined : 'none',
              }}
            >
            ...
            <div>
              ...
              >
                {(['pending', 'in-progress', 'flagged', 'complete'] as const).map((status) => {
                  const count = selectedProject.tasksStore!.tasks.filter(
                    (t) => t.status === status,
                  ).length;
              ...
              >
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      fontSize: '0.85rem',
                      background: boardView === 'summary' ? undefined : 'transparent',
                      boxShadow: boardView === 'summary' ? undefined : 'none',
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      fontSize: '0.85rem',
                      background: boardView === 'kanban' ? undefined : 'transparent',
                      boxShadow: boardView === 'kanban' ? undefined : 'none',
                    }}
                  >
                    <Layout size={14} /> Tablero
                  </button>
                ...
                <span style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                  Resumen del Proyecto — {pctComplete}% completo ({completedTasks}/{totalTasks}{' '}
                  tareas{flaggedTasks > 0 ? `, ${flaggedTasks} con bandera` : ''})
                </span>
              </div>
                  ...
                  {totalTasks === 0 ? (
                    ...
                    <table
                      style={{
                        ...
                        minWidth: orderedStages.length > 4 ? '680px' : 'auto',
                      }}
                    >
                      <thead>
                        <tr>
                                ...
                                >
                                  {status && (
                                    <span
                                      ...
                                      title={`Estado: ${getTaskStatusLabel(status)}`}
                                    >
                                      {CELL_ICONS[status]}
                                      {cellTasks.length > 1 && (
                                        <sup style={{ fontSize: '0.6rem' }}>{cellTasks.length}</sup>
                                      )}
                                    </span>
                                  )}
                                </td>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            className="glass"
                            style={{ fontSize: '0.8rem', padding: '2px 8px', borderRadius: '10px' }}
                          >
                            {colTasks.length}
                          </span>
                        </div>
                          ...
                          {colTasks.length === 0 ? (
                            ...
                            colTasks.map((task) => (
                              <div
                                key={task.id}
                                className="glass"
                                style={{ padding: '16px', background: 'rgba(30, 41, 59, 0.4)' }}
                              >
                                <div
                                  style={{
                                    display: 'flex',
                                    ...
                                    {task.assignedTo?.map((name, idx) => (
                                      <span
                                        ...
                                        style={{
                                          ...
                                          fontSize: '0.7rem',
                                          fontWeight: 600,
                                          background: `hsl(${(idx * 137) % 360}, 50%, 60%)`,
                                          color: '#fff',
                                        }}
                                        title={name}
                                      >
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        <span>
                          Propuesto por: <strong>{selectedPr.author}</strong>
                        </span>
                        <span>Creado: {new Date(selectedPr.createdAt).toLocaleDateString()}</span>
                        <span>
                          Actualizado: {new Date(selectedPr.updatedAt).toLocaleDateString()}
                        </span>
                      </div>
                    </div>
                  ...
                  {(selectedPr.originalText || selectedPr.proposedText) && (
                    <div>
                      ...
                      >
                        Comparativa de Versículos (Diff)
                      </h3>
                      <div
                        className="glass"
                        style={{
                  ...
                  <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '20px' }}>
                    ...
                    >
                      Sugerir Traducción Alternativa
                    </h3>
                              ...
                              >
                                ...
                                <button
                                  ...
                                  style={{
                                    padding: '6px 12px',
                                    fontSize: '0.8rem',
                                    background: userHasVotedAlt
                                      ? 'rgba(16,185,129,0.1)'
                                      : undefined,
                                  }}
                                >
                                  <ThumbsUp size={12} style={{ color: '#10b981' }} /> Upvote (
                                  {alt.votes?.length || 0})
                                </button>
                              </div>
                            );
