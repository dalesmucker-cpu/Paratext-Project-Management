...
async function run() {
  try {
    ...
    const url = 'https://api.github.com/repos/dalesmucker-cpu/Paratext-Project-Management/releases/latest';
    ...
    if (!res.ok) {
      sendResult({ status: 'error', error: `GitHub API request failed: ${res.status} ${res.statusText}` });
      process.exit(1);
    }
    ...
    if (!assetRes.ok) {
      sendResult({ status: 'error', error: `Failed to download release zip: ${assetRes.status} ${assetRes.statusText}` });
      process.exit(1);
    }
    ...
    exec(psCommand, (err, stdout, stderr) => {
      ...
      if (err) {
        sendResult({ status: 'error', error: `Failed to extract update: ${stderr || err.message}` });
        process.exit(1);
      }
