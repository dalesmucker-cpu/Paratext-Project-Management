...
/**
 * Unicode-aware word boundary patterns that handle accented characters (Spanish, etc.) Standard JS
 * \b treats accented letters as non-word characters.
 */

/**
 * Build a Unicode-aware regex for a word, wrapping with lookahead/lookbehind to simulate word
 * boundaries that respect accented letters.
 */
...
export function matchRendering(
  ...
  if (hasAsterisk) {
    ...
    try {
      ...
      const fallbackRegex = new RegExp(
        regexStr.replace(/\\p\{[LN]\}/g, '\\w').replace(/\\p\{N\}/g, '\\d'),
        'i',
      );
  ...
  if (activeInfixes.length > 0) {
    const rawInfixes = activeInfixes
      .map((i) => i.affix.replace(/^-|-$/g, '').trim())
      .filter(Boolean);
    if (rawInfixes.length > 0) {
      ...
      try {
        ...
        // Fallback without Unicode property escapes
        try {
          const fallbackInfixRegex = new RegExp(
            `\\b${escapedChars.join(`${infixPattern}?`)}\\b`,
            'i',
          );
